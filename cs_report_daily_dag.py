...
QUERY = r'''-- Customer Success Report — single-row payload (newest-first comma series, 30d).
...
def render_report(payload, mode):
    ...
    cols=[('Created→OW','ow'),('Esc→Human FRT','hufrt'),('Created→First Res','frt')]
    for r,(tn,sfx) in enumerate(tiers):
    ...
    _chart(cax[2],labels,[{'data':tat_series('frt_p50'),'color':PINK}],'m',0,title='CREATED → FIRST RES')
