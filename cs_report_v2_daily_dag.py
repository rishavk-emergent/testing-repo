"""
CS Customer Success Report v2 — DAILY (image report).
Standalone DAG. Queries BigQuery for the daily payload, builds 3 HTML section docs,
commits them to the render repo under render/latest/. A GitHub Action there
screenshots them and posts the images to Slack (Composer has no headless Chrome).
Schedule: 0 11 * * *  (Asia/Kolkata -> 11 AM IST). Triggers: NONE.
Composer env: CS_REPORT_GH_TOKEN (repo-scope PAT), CS_REPORT_GH_REPO, CS_REPORT_GH_BRANCH.
"""
...
RENDER_DIR = os.getenv('CS_REPORT_RENDER_DIR', 'render/latest')
...
def share_series(numkey, denkey, n=7):
    a = nums(numkey); b = nums(denkey)
    out = []
...
chart_labels = list(reversed(all_labels[:7]))
...
def delta_calc(today, prev, direction):
    ...
    sign = '+' if diff > 0 else ''
    return (col, '<span class="c-delta" style="color:' + col + '">' + arrow + ' ' + sign + '{:.0f}%'.format(pctc) + ' vs prev</span>')
...
try:
    ...
    display_date = date(int(p[0]),int(p[1]),int(p[2])).strftime('%a, %d %b %Y')
except: pass
...
def masthead(sub):
    return ('<div class="masthead"><div><div class="masthead-title">CUSTOMER SUCCESS REPORT</div>'
...
def vol_rows():
    ...
    rows += card('Total Tickets Closed', fmt_int(y('closed')), 'closed (by first-eval day)', y('closed'), yprev('closed'), 'neutral')
...
html_r1 = doc(
    ...
    + '<div class="row-label">&mdash; OW% vs Human% &middot; last 7 days</div>'
...
html_r2 = doc(
    ...
    + '<div class="row-label">&mdash; p75 trend &middot; last 7 days</div>'
...
html_r3 = doc(
    ...
    + '<div class="row-label">&mdash; trends &middot; last 7 days</div>'
...
slack_text = (':bar_chart: *CUSTOMER SUCCESS REPORT — ' + newest_label + '*\n'
...
def run_cs_report(**context):
    ...
    logger.info("CS report v2 daily committed -> %s", out.get('committed', ''))
...
dag = DAG('cs_report_v2_daily', default_args=default_args,
    description='Daily CS Success Report (3 sections) -> commit to render repo -> GitHub Action posts images',
    schedule_interval='0 11 * * *', catchup=False,
    tags=['slack','analytics','cs_metrics','reporting','images'])
PythonOperator(task_id='build_and_commit_cs_report_daily', python_callable=run_cs_report, dag=dag)
